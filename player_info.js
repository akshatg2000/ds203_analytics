         ...
         try {
            const browser = await puppeteer.launch({ headless: "new" });
              const page = await browser.newPage();
                          ...
                          //            });
//                                       console.log(name,teamName,battingStyle,bowlingStyle,playingRole)
                                     playersData.push({
                                     name,
                                     teamName,
                                     battingStyle,
...
//                       link: 'https://www.espncricinfo.com/cricketers/sikandar-raza-299572'
//                     };
    const PlayerData= await performStage3(PlayerLinks);
    console.log(PlayerData)

     //Save the bowling summaries as a JSON file
    const playerinfoJSON = JSON.stringify(PlayerData, null, 2);
    fs.writeFile("player_info.json", playerinfoJSON, (err) => {
      if (err) {
